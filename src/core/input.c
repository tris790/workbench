...
void Input_BeginFrame(void *raw_input_ptr) {
    ...
    memcpy(g_input.raw.mouse_down, raw_input->mouse_down, sizeof(raw_input->mouse_down));
    memcpy(g_input.raw.mouse_pressed, raw_input->mouse_pressed, sizeof(raw_input->mouse_pressed));
    memcpy(g_input.raw.mouse_released, raw_input->mouse_released, sizeof(raw_input->mouse_released));
    g_input.raw.scroll_delta = raw_input->scroll_delta;
    memcpy(g_input.raw.key_down, raw_input->key_down, sizeof(raw_input->key_down));
    memcpy(g_input.raw.key_pressed, raw_input->key_pressed, sizeof(raw_input->key_pressed));
    memcpy(g_input.raw.key_released, raw_input->key_released, sizeof(raw_input->key_released));
...
void Input_ConsumeKeys(void) {
    g_input.key_consumed = true;
}

void Input_ConsumeText(void) {
    g_input.text_consumed = true;
}

void Input_ConsumeMouse(void) {
    g_input.mouse_consumed = true;
}

void Input_ConsumeScroll(void) {
    g_input.scroll_consumed = true;
}

/* ===== Input Queries ===== */

b32 Input_KeyPressed(key_code key) {
    if (g_input.key_consumed) return false;
...
u32 Input_GetTextInput(void) {
    if (g_input.text_consumed) return 0;
    return g_input.raw.text_input;
}

u32 Input_GetModifiers(void) {
    return g_input.raw.modifiers;
}

b32 Input_MousePressed(mouse_button button) {
    if (g_input.mouse_consumed) return false;
...
f32 Input_GetScrollDelta(void) {
    if (g_input.scroll_consumed) return 0.0f;
    return g_input.raw.scroll_delta;
}

v2i Input_GetMousePos(void) {
    return g_input.raw.mouse_pos;
}
