...
#define LATEST_CONFIG_VERSION 2
...
static void SetDefaults(void) {
  Config_SetI64("config_version", LATEST_CONFIG_VERSION);
  ...
  Config_SetString("terminal.shell", ""); /* Default: system default */
  Config_SetString("terminal.shell_mode",
                   "native"); /* native, fish, system, emulated */
}
...
b32 Config_Reload(void) {
  ...
  if (result) {
    if (Config_NeedsUpgrade()) {
      i64 current_version = Config_GetI64("config_version", 0);
      Config_AddDiagnostic(
          "Config version %lld is outdated. A new version (%d) is available.",
          (long long)current_version, LATEST_CONFIG_VERSION);
      Config_AddDiagnostic(
          "Use 'Config: Upgrade' to update your configuration file.");
    }
...
b32 Config_NeedsUpgrade(void) {
  return Config_GetI64("config_version", 0) < LATEST_CONFIG_VERSION;
}
