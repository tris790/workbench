...
void ANSI_Init(ansi_parser *parser) {
  ...
  parser->state = WB_ANSI_STATE_GROUND;
}
...
ansi_result ANSI_Parse(ansi_parser *parser, u8 byte) {
  ansi_result result = {.action = WB_ANSI_ACTION_NONE};

  switch (parser->state) {
  case WB_ANSI_STATE_GROUND:
    if (byte == 0x1B) {
      /* ESC - start escape sequence */
      parser->state = WB_ANSI_STATE_ESCAPE;
    } else if (is_control(byte)) {
      /* Control character */
      result.action = WB_ANSI_ACTION_EXECUTE;
      result.data.control = byte;
    } else {
      /* Printable character */
      result.action = WB_ANSI_ACTION_PRINT;
      result.data.character = byte;
    }
    break;

  case WB_ANSI_STATE_ESCAPE:
    if (byte == '[') {
      /* CSI sequence */
      parser->state = WB_ANSI_STATE_CSI;
  ...
  case WB_ANSI_STATE_CSI:
    if (byte == '?') {
      ...
      result.action = WB_ANSI_ACTION_CSI;
  ...
  case WB_ANSI_STATE_DCS:
    if (byte == 0x1B) {
        /* ESC - potentially start of ST */
        parser->state = WB_ANSI_STATE_DCS_ESCAPE;
    } else if (byte == 0x07) {
        /* BEL - terminates */
        result.action = WB_ANSI_ACTION_DCS;
        parser->state = WB_ANSI_STATE_GROUND;
    }
    /* Otherwise consume and ignore byte */
    break;

  case WB_ANSI_STATE_DCS_ESCAPE:
    if (byte == '\\') {
        /* ESC \ (ST) - terminates */
        result.action = WB_ANSI_ACTION_DCS;
        parser->state = WB_ANSI_STATE_GROUND;
