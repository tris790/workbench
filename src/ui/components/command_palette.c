...
static void PopulateFileItems(command_palette_state *state) {
  ...
  for (u32 i = 0; i < state->fs->entry_count && state->item_count < PALETTE_MAX_ITEMS; i++) {
...
static void PopulateCommandItems(command_palette_state *state) {
  ...
  for (i32 i = 0; i < state->command_count && state->item_count < PALETTE_MAX_ITEMS; i++) {
...
void CommandPalette_RegisterCommand(command_palette_state *state,
  ...
  snprintf(cmd->shortcut, sizeof(cmd->shortcut), "%s", shortcut ? shortcut : "");
  snprintf(cmd->category, sizeof(cmd->category), "%s", category ? category : "");
...
b32 CommandPalette_Update(command_palette_state *state, ui_context *ui) {
  ...
  if (input->key_pressed[KEY_UP] && !ctrl) {
  ...
  if (input->key_pressed[KEY_DOWN] && !ctrl) {
  ...
  i32 text_len = (i32)strlen(state->input_buffer);
  i32 min_cursor = (state->mode == PALETTE_MODE_COMMAND) ? 2 : 0;
  b32 shift = (input->modifiers & MOD_SHIFT) != 0;
  
  /* Ctrl+A - Select all (after prefix in command mode) */
  if (ctrl && input->key_pressed[KEY_A]) {
    state->input_state.selection_start = min_cursor;
    state->input_state.selection_end = text_len;
    state->input_state.cursor_pos = text_len;
    return true;
  }
  ...
  if (ctrl && input->key_pressed[KEY_C]) {
    if (state->input_state.selection_start >= 0) {
      i32 start = state->input_state.selection_start < state->input_state.selection_end
                      ? state->input_state.selection_start
                      : state->input_state.selection_end;
      i32 end = state->input_state.selection_start > state->input_state.selection_end
                    ? state->input_state.selection_start
                    : state->input_state.selection_end;
      char temp[PALETTE_MAX_INPUT];
      strncpy(temp, state->input_buffer + start, (size_t)(end - start));
      temp[end - start] = '\0';
  ...
  if (ctrl && input->key_pressed[KEY_X]) {
    if (state->input_state.selection_start >= 0) {
      ...
      i32 end = state->input_state.selection_start > state->input_state.selection_end
                    ? state->input_state.selection_start
                    : state->input_state.selection_end;
      /* Clamp start to min_cursor */
      if (start < min_cursor) start = min_cursor;
      ...
      /* Delete selection */
      memmove(&state->input_buffer[start], &state->input_buffer[end],
              (size_t)(text_len - end + 1));
      state->input_state.cursor_pos = start;
  ...
  if (ctrl && input->key_pressed[KEY_V]) {
    /* Get clipboard content (using fs arena if available) */
    char *clipboard = NULL;
    if (state->fs && state->fs->arena) {
      clipboard = Platform_GetClipboard(state->fs->arena);
    }
    ...
    if (clipboard && clipboard[0] != '\0') {
      ...
      if (state->input_state.selection_start >= 0) {
        ...
        state->input_state.cursor_pos = start;
        state->input_state.selection_start = -1;
        text_len = (i32)strlen(state->input_buffer);
      }
      
      /* Insert clipboard content at cursor */
      i32 paste_len = (i32)strlen(clipboard);
      if (text_len + paste_len < PALETTE_MAX_INPUT - 1) {
        i32 cursor = state->input_state.cursor_pos;
        memmove(&state->input_buffer[cursor + paste_len],
                &state->input_buffer[cursor],
                (size_t)(text_len - cursor + 1));
        memcpy(&state->input_buffer[cursor], clipboard, (size_t)paste_len);
        state->input_state.cursor_pos += paste_len;
      }
    }
    return true;
  }
  ...
  if (ctrl && input->key_pressed[KEY_BACKSPACE]) {
    if (state->input_state.cursor_pos > min_cursor) {
      i32 cursor = state->input_state.cursor_pos;
      i32 word_start = cursor - 1;
      
      /* Skip whitespace first */
      while (word_start > min_cursor && state->input_buffer[word_start - 1] == ' ') {
        word_start--;
      }
      ...
      memmove(&state->input_buffer[word_start],
              &state->input_buffer[cursor],
              (size_t)(text_len - cursor + 1));
  ...
  if (ctrl && input->key_pressed[KEY_DELETE]) {
    if (state->input_state.cursor_pos < text_len) {
      i32 cursor = state->input_state.cursor_pos;
      i32 word_end = cursor;
      
      /* Skip non-whitespace (the word) first */
      while (word_end < text_len && state->input_buffer[word_end] != ' ') {
        word_end++;
      }
      ...
      memmove(&state->input_buffer[cursor],
              &state->input_buffer[word_end],
              (size_t)(text_len - word_end + 1));
      state->input_state.selection_start = -1;
    }
    return true;
  }
  
  /* Left arrow - move cursor left (with shift for selection) */
  if (input->key_pressed[KEY_LEFT]) {
    if (shift && state->input_state.selection_start < 0) {
      state->input_state.selection_start = state->input_state.cursor_pos;
    }
    if (state->input_state.cursor_pos > min_cursor) {
      state->input_state.cursor_pos--;
    }
    if (shift) {
      state->input_state.selection_end = state->input_state.cursor_pos;
    } else {
      state->input_state.selection_start = -1;
    }
    return true;
  ...
  /* Right arrow - move cursor right (with shift for selection) */
  ...
  if (input->key_pressed[KEY_HOME]) {
    ...
    state->input_state.cursor_pos = min_cursor;
    if (shift) {
      state->input_state.selection_end = state->input_state.cursor_pos;
    } else {
      state->input_state.selection_start = -1;
  ...
  /* Delete - delete character at cursor or selection */
  if (input->key_pressed[KEY_DELETE]) {
    if (state->input_state.selection_start >= 0) {
      ...
      state->input_state.cursor_pos = start;
      state->input_state.selection_start = -1;
    } else if (state->input_state.cursor_pos < text_len) {
      /* Remove character at cursor */
      memmove(&state->input_buffer[state->input_state.cursor_pos],
              &state->input_buffer[state->input_state.cursor_pos + 1],
              (size_t)(text_len - state->input_state.cursor_pos));
    }
    return true;
  }

  /* Handle text input (typing characters) */
  if (input->text_input && input->text_input >= 32 && input->text_input < 128) {
    /* Delete selection first if any */
    if (state->input_state.selection_start >= 0) {
      i32 start = state->input_state.selection_start < state->input_state.selection_end
                      ? state->input_state.selection_start
    ...
    if (text_len < PALETTE_MAX_INPUT - 1) {
      /* Insert at cursor position */
      i32 cursor = state->input_state.cursor_pos;
      memmove(&state->input_buffer[cursor + 1],
              &state->input_buffer[cursor],
              (size_t)(text_len - cursor + 1));
      state->input_buffer[cursor] = (char)input->text_input;
      state->input_state.cursor_pos++;
    }
    return true;
  }
  ...
  if (input->key_pressed[KEY_BACKSPACE]) {
    if (state->input_state.selection_start >= 0) {
      ...
      state->input_state.cursor_pos = start;
      state->input_state.selection_start = -1;
    } else if (state->input_state.cursor_pos > min_cursor) {
  ...
  static char last_input[PALETTE_MAX_INPUT] = {0};
  if (strcmp(state->input_buffer, last_input) != 0) {
    snprintf(last_input, sizeof(last_input), "%s", state->input_buffer);
...
void CommandPalette_Render(command_palette_state *state, ui_context *ui,
  ...
  i32 visible_items = state->item_count < max_visible_items ? state->item_count : max_visible_items;
  ...
  Render_DrawRectRounded(renderer, state->panel_bounds, th->radius_md, panel_bg);
  ...
  rect border_rect = {panel_x - 1, panel_y - 1, panel_width + 2, panel_height + 2};
  Render_DrawRectRounded(renderer, border_rect, th->radius_md + 1, border_color);
  Render_DrawRectRounded(renderer, state->panel_bounds, th->radius_md, panel_bg);

  /* ===== Input Field ===== */
  rect input_rect = {panel_x + 8, panel_y + 4, panel_width - 16, input_height - 4};
  ...
  i32 padding = 8;
  v2i text_pos = {input_rect.x + padding, input_rect.y + (input_height - 4) / 2 - 8};

  /* Draw selection highlight if any */
  if (state->input_state.selection_start >= 0 && state->input_buffer[0] != '\0') {
    i32 sel_start = state->input_state.selection_start < state->input_state.selection_end
                        ? state->input_state.selection_start
                        : state->input_state.selection_end;
    i32 sel_end = state->input_state.selection_start > state->input_state.selection_end
                      ? state->input_state.selection_start
                      : state->input_state.selection_end;
  ...
  if (state->input_buffer[0] != '\0') {
    ...
    const char *hint = state->mode == PALETTE_MODE_FILE ? 
                       "Search files..." : 
                       "Type a command...";
  ...
  if (state->just_opened || CommandPalette_IsOpen(state)) {
    ...
    if (show_cursor) {
      ...
      Render_DrawRect(renderer, (rect){cursor_x, text_pos.y, 2, 18}, th->accent);
    }
  }
  ...
  Render_DrawRect(renderer, (rect){panel_x, sep_y, panel_width, 1}, border_color);
  ...
  for (i32 i = 0; i < state->item_count; i++) {
    ...
    i32 item_y = list_rect.y + i * state->item_height - (i32)state->scroll.offset.y;

    /* Skip items outside visible area */
    if (item_y + state->item_height < list_rect.y || item_y > list_rect.y + list_height)
      continue;
    ...
    if (item->shortcut[0] != '\0') {
      ...
      Render_DrawText(renderer, (v2i){shortcut_x, text_y}, item->shortcut, f, shortcut_color);
    }
    ...
    if (item->shortcut[0] == '\0' && item->category[0] != '\0') {
      ...
      Render_DrawText(renderer, (v2i){cat_x, text_y}, item->category, f, cat_color);
    }
  }
