...
void ConfigDiagnostics_Render(ui_context *ui, rect bounds,
  ...
  Render_DrawText(ctx, title_pos, "Configuration Diagnostics", ui->main_font,
                  th->text);
  ...
  rect path_rect = {
      ...
      dialog.w - th->spacing_lg * 2, Font_GetLineHeight(ui->main_font)};

  /* Config Path */
  char path_buf[1024];
  snprintf(path_buf, sizeof(path_buf), "File: %s",
           Config_GetPath() ? Config_GetPath() : "Unknown");
  Render_DrawText(ctx, (v2i){path_rect.x, path_rect.y}, path_buf, ui->main_font,
                  th->text);

  /* Scrollable content area - below path */
  i32 path_area_h = Font_GetLineHeight(ui->main_font) + 12;
  ...
  /* Measure buttons */
  i32 close_w = UI_MeasureText("Close", ui->main_font).x + btn_pad;
  if (close_w < 80)
    close_w = 80; /* Min width for aesthetics */

  i32 open_w = UI_MeasureText("Open Config File", ui->main_font).x + btn_pad;
  i32 reload_w = UI_MeasureText("Reload", ui->main_font).x + btn_pad;
